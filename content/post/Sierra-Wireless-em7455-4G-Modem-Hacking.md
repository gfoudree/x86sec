...
![Firmware webui](/firmware_webui.webp)
...
![Auth](/modem_auth_mechanism.webp)

To perform the hash, the binary uses the `crypt()` function

![hash](/modem_hash_func.webp)
...
for m in re.findall(b'\x00+([!+$>^][A-Z]+)\x00+', d):
    print(m.decode())
```
The list is too long, so I am posting them all here: [AT_commands.txt](/AT_commands.txt)
